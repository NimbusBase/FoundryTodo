...
(function() {
      ...
      return require(main.plugins, function() {
        ...
        angular.module('enterprise', dependency).config([
          ...
          '$rootScope', '$location', function($rootScope, $location) {
            ...
            $rootScope._plugins = [];
            _ref = enterprise._plugins;
            for (inex in _ref) {
              _plugin = _ref[inex];
              $rootScope._plugins.push(_plugin);
            }
            $rootScope._active_app_path = '';
            $rootScope._current_global_user = enterprise._current_user;
            $rootScope.$on('$locationChangeSuccess', function(evt, new_path, old_path) {
              var default_path;
              default_path = localStorage.default_plugin || enterprise.default_plugin;
  ...
  core.set_setting = function(key, value) {
    var my_settings, x;
    my_settings = enterprise.settings.select(function(item) {
      return item.userid === enterprise._current_user.id && item.setting_name === key;
    });
    if (my_settings.length === 0) {
      return enterprise.settings.create({
        "userid": enterprise._current_user.id,
        "setting_name": key,
        "setting_value": value
      });
    } else {
  ...
  core.get_setting = function(key) {
    var setting;
    setting = enterprise.settings.select(function(item) {
      return item.userid === enterprise._current_user.id && item.setting_name === key;
    });
    if (setting.length > 0) {
  ...
  define('document', function(require) {
    ...
    return doc_plugin = {
      ...
      init: function() {
        ...
        enterprise.model(name, attributes, function(model) {
          ...
          return self.all_file(function() {
            return enterprise.initialized(self.name);
          });
        });
        define_controller();
      },
      ...
      set: function(id, data) {
        ...
        if (!this._documents[id]) {
          ...
          model = enterprise._models.Document;
          doc = model.create({
            title: data.title,
            id: data.id,
  ...
  define_controller = function() {
    return angular.module('enterprise').controller('DocumentController', [
      '$scope', '$rootScope', 'ngDialog', '$enterprise', '$timeout', function($scope, $rootScope, ngDialog, $enterprise, $timeout) {
        ...
        file_module = enterprise.load('document');
    ...
    return user_plugin = {
      ...
      init: function() {
        ...
        enterprise.model(name, attributes, function(model) {
          ...
          enterprise.initialized(self.name);
        });
      },
      inited: function() {
        ...
        _ref = enterprise._user_list;
        for (id in _ref) {
          user = _ref[id];
          if (user.id === c_file.owners[0].permissionId) {
            enterprise._current_owner = user;
          }
        }
        this.get_user_plan(function(plan) {
          ...
          if (enterprise._current_user.id !== enterprise._current_owner.id) {
            return;
          }
          if (status === 0) {
      ...
      check_users: function() {
        ...
        user_model = enterprise._models['User'];
        console.log('user list total :' + keys(enterprise._user_list).length + ', user model total: ' + user_model.all().length);
        _ref = user_model.all();
        ...
        for (pid in _ref1) {
          ...
          if (one && !one.pic) {
            ...
            if (pid === enterprise._current_user.id) {
              data.email = window.user_email;
            }
            data.debug = user_email + ':' + 'time: ' + new Date() + JSON.stringify(enterprise._models.User.all());
      ...
      add_user: function(data, callback) {
        ...
        return analytic.user({
          email: enterprise._current_user.email,
          user: data.email,
          id: enterprise._current_user.id
        });
      },
      ...
      mail_list: function() {
        ...
        for (id in _ref) {
          user = _ref[id];
          if (user.email === enterprise._current_user.email) {
            continue;
          }
          if ((enterprise.get_setting_all(id, "email") != null) && enterprise.get_setting_all(id, "email") === false) {
            continue;
          }
          if (i === 0 && user.email) {
      ...
      get_user_plan: function(callback) {
        var self, url;
        self = this;
        url = "http://192.241.167.76:4000/order/" + enterprise._current_owner.id + "/" + enterprise._current_owner.email;
        return $.ajax({
          'url': url,
          success: function(data) {
            ...
            if (data) {
              user = data;
              self.user_plan = enterprise._owner_plan = Number(user.order.type);
            } else {
              user = false;
              self.user_plan = enterprise._owner_plan = 0;
            }
            if (callback) {
  ...
  inject_controller = function() {
    return angular.module('enterprise').controller('UserListController', [
      '$scope', '$rootScope', '$parse', function($scope, $rootScope, $parse) {
        ...
        user_model = enterprise.load('user');
        $scope.users = enterprise._user_list;
        ...
        current_user = enterprise._current_user;
        $scope.user_permission = 'Viewer';
        update_current_user_permission = function() {
          var current_user_in_model;
          current_user_in_model = enterprise._models.User.findByAttribute('pid', current_user.id);
        ...
        $scope.del_user = function(id) {
          ...
          return delete enterprise._user_list[id];
        };
        ...
        $scope.is_owner = function(user) {
          return user.id === enterprise._current_owner.id;
        };
    ...
    return doc_plugin = {
      ...
      init: function() {
        ...
        if (localStorage['last_opened_workspace'] && (localStorage['last_opened_workspace'] !== c_file.id)) {
          ...
          enterprise.shared_users(function(users) {
            var _users;
            _users = users;
            return enterprise.current_user(function(me) {
              var user, _i, _len;
              for (_i = 0, _len = _users.length; _i < _len; _i++) {
                user = _users[_i];
                if (user.id === me.id) {
                  enterprise._current_user.role = user.role;
                }
              }
              if (!enterprise._current_user.email) {
                enterprise._current_user.email = Nimbus.Share.get_user_email();
              }
              console.log(_users);
              return enterprise.initialized(self.name);
            });
      ...
      open: function(doc, callback) {
        ...
        Nimbus.Share.switch_to_app_file_real(doc.id, function() {
          ...
          ga('set', 'dimension4', enterprise._models.User.all());
        });
      },
  ...
  define_controller = function() {
    return angular.module('enterprise').controller('ProjectController', [
      '$scope', '$rootScope', 'ngDialog', '$enterprise', function($scope, $rootScope, ngDialog, $enterprise) {
        var docModule;
        docModule = enterprise.load('workspace');
  ...
  define('core/analytic', function() {
    ...
    init = function() {
      ga('create', 'UA-46950334-2', {
        'userId': enterprise._current_user.email
      });
      ga('require', 'displayfeatures');
      ga('set', 'dimension1', enterprise._current_user.email);
      ga('set', 'dimension2', c_file.title);
      ga('set', 'dimension3', c_file.owners[0].emailAddress + ':' + c_file.owners[0].displayName);
    ...
    get_owner_space_count = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        space = _ref[_i];
        if (space.owners[0].permissionId === enterprise._current_user.id) {
          count++;
        }
      }
      return count;
    ...
    send_user_event = function(data) {
      ...
      count = enterprise._models.User.all().length;
  ...
  angular.module('enterpise-ui', ['ngDialog']).directive('enEditor', [
    '$parse', '$timeout', function($parse, $timeout) {
      return function(scope, elm, attrs) {
        ...
        editor = $(elm).wysihtml5($.extend(enterprise.wysiwygOptions, {
          stylesheets: [],
          events: {
            'change': update,
            'blur': update
      ...
      service = {
        ...
        gmail: function(subject, to, content, cc, bcc) {
          ...
          mailCallback = function(error, email) {
            ...
            currentUserEmail = enterprise._current_user.email;
